...
let region_spots = read.csv(system.file("data/HR2MSI mouse urinary bladder_outline.csv", package = "MSImaging"), row.names = NULL);
...
bitmap(file = `${@dir}/HR2MSI_mouse_urinary_bladder_S096_regionMSI.png`, size = [3300, 2000]) {
    ...
    ggplot(open.mzpack(msi_rawdata), 
       ...
       + geom_sample_outline(spots = region_spots,
            ...
            resolution = 1000,
            q = 0.1,
            line_stroke = "stroke: blue; stroke-width: 6px; stroke-dash: solid;")
